...
def MakeIXPosition(PC):
    ...
    # temp1_z = np.concatenate((zero_xy,temp),axis=2)         # 薄さ0の導体なのでz方向にはない
    # temp2_z = np.concatenate((temp,zero_xy),axis=2)         # 薄さ0の導体なのでz方向にはない
    # temp    = temp1_z+temp2_z
    temp = np.concatenate((temp,zero_xy),axis=2)
    for i in range(nz+1):
