...
nFrame = 300
rate = 3
vmax=Ey_xz[100,:,:].max()/3
vmin=-Ey_xz[100,:,:].max()/3
...
def update(i):
    ...
    print(time)
    ### Plot ###
    plt.imshow(Ey_xz[:,:,time].T,vmax=vmax,vmin=vmin,cmap="bwr",origin="lower")
    plt.tight_layout()
ani = animation.FuncAnimation(fig, update,frames=int(nFrame/rate))
ani.save("Movie_Ey_xz_"+CirName+".gif", writer="imagemagick")
